<?php
// $Id: index.php,v 1.1 2004/11/23 22:15:09 praedator Exp $
//  ------------------------------------------------------------------------ //
...
function get_subcat_data($parent, &$cat_data, &$album_set_array, $level, $ident = '')
{
    ...
    if ($xoopsDB->getRowsNum($sql) > 0) {
        ...
        while (false !== ($subcat = $xoopsDB->fetchArray($sql))) {
            if (USER_GAL_CAT == $subcat['cid']) {
                ...
                $link = $ident . "<a href=index.php?cat={$subcat['cid']}>" . htmlspecialchars($subcat['name'], ENT_QUOTES | ENT_HTML5) . '</a>';

                if ($album_count) {
                    $cat_data[] = [$link, $ident . $subcat['description'], $album_count, $pic_count];
                ...
                $subcat['name'] = htmlspecialchars($subcat['name'], ENT_QUOTES | ENT_HTML5);

                $subcat['description'] = preg_replace("/<br \>/i", '<br>' . $ident, $myts->displayTarea($subcat['description']));

                $link = $ident . "<a href=\"index.php?cat={$subcat['cid']}\">{$subcat['name']}</a>";

                if (0 == $pic_count && 0 == $album_count) {
                    $cat_data[] = [$link, $ident . $subcat['description']];
                } else {
                    $cat_data[] = [$link, $ident . $subcat['description'], $album_count, $pic_count];
                }
            }
            ...
            if ($level > 1) {
                get_subcat_data($subcat['cid'], $cat_data, $album_set_array, $level - 1, $ident . '<img src="images/spacer.gif" width="20" height="1" alt="">');
            }
        }
    }
}
...
function list_users()
{
    ...
    for ($i = $lower_limit; $i < $upper_limit; $i++) {
        ...
        $alb_owner = $userHandler->getUser($user['category'] - FIRST_USER_CAT);
...
function list_albums()
{
    ...
    $album_set = '(' . mb_substr($album_set, 0, -2) . ')';
    ...
    for ($alb_idx = 0; $alb_idx < $disp_album_count; $alb_idx++) {
        ...
        if ($count > 0) {
            ...
            if ('0' == $visibility || $visibility == (FIRST_USER_CAT + USER_ID) || (is_array($USER_DATA['group_id']) && in_array($visibility, $USER_DATA['group_id'], true))) {
        ...
        $alb_list[$alb_idx]['album_title'] = htmlspecialchars($alb_thumb['title'], ENT_QUOTES | ENT_HTML5);

        $alb_list[$alb_idx]['album_desc'] = $myts->displayTarea($alb_thumb['description'], 1);
...
$elements = preg_preg_split('|/|', $xoopsModuleConfig['main_page_layout'], -1, PREG_SPLIT_NO_EMPTY);
...
if (RANDPOS_INTERVAL != $granularity && $pic_count > RANDPOS_MAX_PIC) {
