<?php
// $Id: editpics.php,v 1.1 2004/11/23 22:15:09 praedator Exp $
//  ------------------------------------------------------------------------ //
...
if (UPLOAD_APPROVAL_MODE && !USER_IS_ADMIN) {
    redirect_header('index.php', 2, _MD_ACCESS_DENIED);
}
...
if ('' != $xoopsModuleConfig['user_field2_name']) {
    $THUMB_ROWSPAN++;
}
if ('' != $xoopsModuleConfig['user_field3_name']) {
    $THUMB_ROWSPAN++;
}
if ('' != $xoopsModuleConfig['user_field4_name']) {
    $THUMB_ROWSPAN++;
}
...
function get_post_var($var, $pid)
{
    ...
    if (!isset($_POST[$var_name])) {
        redirect_header('index.php', 2, _MD_PARAM_MISSING . " ($var_name)");
    }
...
function process_post_data()
{
    ...
    foreach ($pid_array as $pid) {
        ...
        $update .= ", caption = '" . $myts->addSlashes($caption, 0) . "'";
        ...
        if ($delete) {
            ...
            $thumb_dir = $xoopsModuleConfig['fullpath'] . 'thumbnails/' . $pic['filepath'];
            ...
            $files = [$dir . $file, $thumb_dir . $xoopsModuleConfig['normal_pfx'] . $file, $thumb_dir . $xoopsModuleConfig['thumb_pfx'] . $file];
...
function form_pic_info()
{
    ...
    if (UPLOAD_APPROVAL_MODE) {
        ...
        $pic_owner = $userHandler->getUser($CURRENT_PIC['owner_id']);
    ...
    $filename = htmlspecialchars($CURRENT_PIC['filename'], ENT_QUOTES | ENT_HTML5);
}
...
$s50 = 50 == $count ? 'selected="selected"' : '';
$s75 = 75 == $count ? 'selected="selected"' : '';
$s100 = 100 == $count ? 'selected="selected"' : '';
...
if (UPLOAD_APPROVAL_MODE) {
    ...
    $sql = 'SELECT * ' . 'FROM ' . $xoopsDB->prefix('xcgal_pictures') . ' ' . //"LEFT JOIN ".$xoopsDB->prefix("users")." AS u ON owner_id = uid ".
...
$GLOBALS['xoopsOption']['template_main'] = 'xcgal_editpics.html';
...
while (false !== ($CURRENT_PIC = $xoopsDB->fetchArray($result))) {
    if (USER_IS_ADMIN) {
    ...
    $GLOBALS["caption{$CURRENT_PIC['pid']}"] = htmlspecialchars($CURRENT_PIC['caption'], ENT_QUOTES | ENT_HTML5);
    ...
    $value_field1 = htmlspecialchars($CURRENT_PIC['user1'], ENT_QUOTES | ENT_HTML5);

    $name_field1 = 'user1' . $CURRENT_PIC['pid'];

    $value_field2 = htmlspecialchars($CURRENT_PIC['user2'], ENT_QUOTES | ENT_HTML5);

    $name_field2 = 'user2' . $CURRENT_PIC['pid'];

    $value_field3 = htmlspecialchars($CURRENT_PIC['user3'], ENT_QUOTES | ENT_HTML5);

    $name_field3 = 'user3' . $CURRENT_PIC['pid'];

    $value_field4 = htmlspecialchars($CURRENT_PIC['user4'], ENT_QUOTES | ENT_HTML5);
    ...
    $xoopsTpl->append(
        ...
        [
            ...
            'title_value' => htmlspecialchars($CURRENT_PIC['title'], ENT_QUOTES | ENT_HTML5),
            'xoops_codes' => $xoops_codes,
            'xoops_smilies' => $smilies,
            'keywords_value' => htmlspecialchars($CURRENT_PIC['keywords'], ENT_QUOTES | ENT_HTML5),
