<?php
// $Id: delete.php,v 1.1 2004/11/23 22:15:09 praedator Exp $
//  ------------------------------------------------------------------------ //
...
function output_caption()
{
    ?>
    <tr>
        <td colspan="6" class="even">&nbsp;</td>
    </tr>
    <tr>
        <td colspan="6" class="odd"><b><?php echo _MD_DEL_CAPTION ?></b>
    </tr>
    <tr>
        <td colspan="6" class="even">
            <table cellpadding="1" cellspacing="0">
                <tr>
                    <td><b>F</b></td>
                    <td>:</td>
                    <td><?php echo _MD_DEL_FS_PIC ?></td>
                    <td width="20">&nbsp;</td>
                    <td><img src="images/green.gif" border="0" width="12" height="12" align="middle" alt=""></td>
                    <td>:</td>
                    <td><?php echo _MD_DEL_DEL_SUCCESS ?></td>
                </tr>
                <tr>
                    <td><b>N</b></td>
                    <td>:</td>
                    <td><?php echo _MD_DEL_NS_PIC ?></td>
                    <td width="20">&nbsp</td>
                    <td><img src="images/red.gif" border="0" width="12" height="12" align="middle" alt=""></td>
                    <td>:</td>
                    <td><?php echo _MD_DEL_ERR_DEL ?></td>
                </tr>
                <tr>
                    <td><b>T</b></td>
                    <td>:</td>
                    <td><?php echo _MD_DEL_THUMB ?></td>
                </tr>
                <tr>
                    <td><b>C</b></td>
                    <td>:</td>
                    <td><?php echo _MD_DEL_COMMENT ?></td>
                </tr>
                <tr>
                    <td><b>D</b></td>
                    <td>:</td>
                    <td><?php echo _MD_DEL_IMGALB ?></td>
                </tr>
            </table>
        </td>
    </tr>
...
function delete_picture($pid)
{
    ...
    if (USER_IS_ADMIN) {
        ...
        $query = 'SELECT '
                  ...
                  . '.aid as aid, category, filepath, filename FROM '
                  . $xoopsDB->prefix('xcgal_pictures')
                  . ', '
                  . $xoopsDB->prefix('xcgal_albums')
                  . ' WHERE '
                  . $xoopsDB->prefix('xcgal_pictures')
                  . '.aid = '
                  . $xoopsDB->prefix('xcgal_albums')
    ...
    if (!is_writable($dir)) {
        redirect_header('index.php', 2, sprintf(_MD_DIRECTORY_RO, htmlspecialchars($dir, ENT_QUOTES | ENT_HTML5)));
    }

    $del_pic = '<tr><td class="even">' . htmlspecialchars($file, ENT_QUOTES | ENT_HTML5) . '</td>';
...
function delete_album($aid)
{
    ...
    while (false !== ($pic = $xoopsDB->fetchArray($result))) {
...
function parse_select_option($value)
{
    ...
    return [
        ...
        'album_nm' => htmlspecialchars($matches[2], ENT_QUOTES | ENT_HTML5),
...
{
    return preg_preg_split('/,/', $value, -1, PREG_SPLIT_NO_EMPTY);
}
...
switch ($what) {
    ...
    case 'albmgr':
        ...
        foreach ($to_delete as $album_id) {
            ...
            $data[] = ['del_message' => $del_message, 'pic_del' => $pic_del];
        }
        $create_update = '';
        if (isset($_POST['to'])) {
    ...
    case 'picture':
        ...
        $data = [];
        $data[] = ['del_message' => '', 'pic_del' => $del_pic];
...
if ($out_caption) {
    $GLOBALS['xoopsOption']['template_main'] = 'xcgal_delete.html';
    ...
    foreach ($data as $dels) {
        $xoopsTpl->append('deletes', ['del_message' => $dels['del_message'], 'pic_del' => $dels['pic_del']]);
    }
