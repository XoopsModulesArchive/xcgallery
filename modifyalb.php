<?php
// $Id: modifyalb.php,v 1.1 2004/11/23 22:15:09 praedator Exp $
//  ------------------------------------------------------------------------ //
...
function get_subcat_data($parent, $ident = '')
{
    ...
    if ($xoopsDB->getRowsNum($result) > 0) {
        ...
        foreach ($rowset as $subcat) {
            $CAT_LIST[] = [$subcat['cid'], $ident . $subcat['name']];
...
function alb_list_box()
{
    ...
    if (GALLERY_ADMIN_MODE) {
        $sql = 'SELECT category, aid, title ' . 'FROM ' . $xoopsDB->prefix('xcgal_albums') . ' ' . //"LEFT JOIN ".$xoopsDB->prefix("users")." AS u ON category = (".FIRST_USER_CAT." + uid) ".
                  'ORDER BY category';
    ...
    if ($xoopsDB->getRowsNum($result) > 0) {
        ...
        while (false !== ($row = $xoopsDB->fetchArray($result))) {
            if ($row['category'] > FIRST_USER_CAT) {
                $alb_owner = $userHandler->getUser($row['category'] - FIRST_USER_CAT);

                if (is_object($alb_owner)) {
                    $row['title'] = '(' . $alb_owner->uname() . ') ' . $row['title'];
                }
            } else {
                $row['title'] = '-' . $row['title'];
            }
...
if (!isset($_GET['album'])) {
    ...
    if (0 == $xoopsDB->getRowsNum($results)) {
        redirect_header('index.php', 2, _MD_MODIFYALB_ERR_NO_ALB);
    }
...
$GLOBALS['xoopsOption']['template_main'] = 'xcgal_modifyalb.html';
...
if (!GALLERY_ADMIN_MODE || $ALBUM_DATA['category'] > FIRST_USER_CAT) {
    ...
    $CAT_LIST[] = [0, _MD_MODIFYALB_NO_CAT];
...
if (0 == $xoopsDB->getRowsNum($results)) {
    ...
    $img_list = [0 => _MD_MODIFYALB_LAST_UPL];

    $pic_url = '';

    $xoopsTpl->assign('no_pic', '');

    while (false !== ($picture = $xoopsDB->fetchArray($results))) {
        $thumb_url = get_pic_url($picture, 'thumb');

        $pic_url .= "Pic[{$picture['pid']}] = '" . $thumb_url . "'\n";

        if ($picture['pid'] == $ALBUM_DATA['thumb']) {
            $initial_thumb_url = $thumb_url;
        }

        $img_list[$picture['pid']] = htmlspecialchars($picture['filename'], ENT_QUOTES | ENT_HTML5);
    }
...
if (!$xoopsModuleConfig['allow_private_albums']) {
    ...
    if (GALLERY_ADMIN_MODE) {
        $options = [0 => _MD_MODIFYALB_PUB_ALB, FIRST_USER_CAT + USER_ID => _MD_MODIFYALB_ME_ONLY];
        ...
        while (false !== ($group = $xoopsDB->fetchArray($result))) {
        ...
        foreach ($usergroups as $ugr) {
            $group = $memberHandler->getGroup($ugr);
...
if (USER_ADMIN_MODE) {
    ...
    $value = $ALBUM_DATA['uploads'] ?? false;

    $yes_selected_upload = 'YES' == $value ? 'selected="selected"' : '';

    $no_selected_upload = 'NO' == $value ? 'selected="selescted"' : '';
...
$value = $ALBUM_DATA['comments'] ?? false;
$yes_selected_comments = 'YES' == $value ? 'selected="selected"' : '';
$no_selected_comments = 'NO' == $value ? 'selected="selescted"' : '';
...
$value = $ALBUM_DATA['votes'] ?? false;
$yes_selected_votes = 'YES' == $value ? 'selected="selected"' : '';
$no_selected_votes = 'NO' == $value ? 'selected="selescted"' : '';
